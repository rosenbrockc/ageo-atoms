#!/usr/bin/env python3
"""Run deterministic runtime probes for the safe Phase 5 subset."""
...
from auditlib.runtime_probes import write_runtime_probe
...
def main() -> None:
    ...
    for record in manifest.get("atoms", []):
        if args.atom_id and record["atom_id"] != args.atom_id:
            continue
        write_runtime_probe(record)
        processed += 1
    print(f"Wrote runtime probe evidence for {processed} atom(s)")
