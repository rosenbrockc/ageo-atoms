...
@icontract.ensure(lambda result, **kwargs: result is not None, "mala_proposal_adjustment output must not be None")
def mala_proposal_adjustment(step_size: float, vals_bound: Any, mala_mean_fn: Callable) -> Any:
...
def mala_proposal_adjustment_ffi(step_size, vals_bound, mala_mean_fn):
