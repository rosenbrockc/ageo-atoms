...
def christov_qrs_segmenter(signal: np.ndarray, sampling_rate: float) -> np.ndarray:  # type: ignore[type-arg]
    """Detects QRS complex onset and offset positions in an ECG signal using the Christov real-time algorithm, which applies a series of signal transformations and adaptive thresholding to locate heartbeat boundaries.
