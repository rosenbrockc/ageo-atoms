...
def bandpass_filter_ffi(signal):
...
def r_peak_detection_ffi(filtered):
...
def peak_correction_ffi(filtered, rpeaks):
...
def template_extraction_ffi(filtered, rpeaks):
...
def heart_rate_computation_ffi(rpeaks):
